...
p_base = ggplot(plot_prop,
  ...
  xlab("EGIDS") +
  scale_fill_manual(values = c(rep("#009E73", 7), 
                               "#E69F00", rep("#D55E00", 4))) +
...
ggsave(plot = pp, filename = "figures/figure1.png")
...
hextri_datacleaning = function(df, languages = languages){
  # ensure the same result everytime
  set.seed(1234)
...
ggsave(plot = p3, filename = "figures/hexbinplots_count.png", 
       height = 297 / 1.95, 
       width = 210, 
       units = "mm")
...
pb.s = length(papercount_pb$value)
gb.s = length(papercount_gb$value)

cat("In Phoible plots there are", pb.s, "cells.\n")
cat("In Grambank plots there are", gb.s, "cells.\n")

# how many cells have been studied
pb.cs = sum(papercount_pb$value != 0)
gb.cs = sum(papercount_gb$value != 0)

cat("In Phoible plots,", pb.cs / pb.s, "cells have been studied")
cat("In Phoible plots,", gb.cs / gb.s, "cells have been studied")

# n papers in one cell
cat("In the Phoible plot", 
    round(max(papercount_pb$value) / sum(papercount_pb$value), 2), 
    "of papers are in the most populated cell.\n")

cat("In the Grambank plot,",
    round(max(papercount_gb$value) / sum(papercount_gb$value), 2),
    "of papers are in the most populated cell.\n")
...
cat("There are", pb.10, "cells in Phoible that have less than 10 papers (",  
    scales::percent(pb.10 / pb.cs), "). From a total of ,", pb.cs, " studied cells\n", 
    "and there are", gb.10, 
    "cells in Grambank (", scales::percent(gb.10 / gb.cs), "). Froma total of", gb.cs, "studied cells.\n")
...
### Hexbin Variation tests

Our Hexbin statistics are based on the number of languages within each hexbin in the graph. Size of hexbins are a parameter chosen by us, however. Here, we show that increasing the size of hexbins only affects the counts but not proportion of cells that are studied or unstudied, with these variables showing a strong positive correlation.
...
bin_size = seq(from = 5, to = 40, by = 5)
...
for(i in seq_along(bin_size)){
  ...
  p3_phoible = ggplot(plot_phoible, 
                    aes(MDS.X, MDS.Y, fill = factor(children), 
                        col = factor(children))) + 
  geom_hextri(color = "white", linewidth = 0.0, bins = bin_size[i]) + 
  geom_hex(fill = NA, color = "white", bins = bin_size[i], lwd = 0.05) +
  stat_summary_hex(aes(
    z = phonology_sum,
    linewidth = after_stat(value),
  ), fun = ~ sum((.x[.x > 0])), col = "black", fill = NA, bins = bin_size[i]) +
  coord_equal() + 
  theme_classic(base_size = font_size) + 
  theme(aspect.ratio = 1, legend.position = "right") + 
  scale_linewidth(range = c(0, 3)) + 
  ...
  pb_pdata.2 = layer_data(p3_phoible, 2)
  gb_pdata.2 = layer_data(p3_grambank, 2)
  
  pb_pdata.3 = layer_data(p3_phoible, 3)
  gb_pdata.3 = layer_data(p3_grambank, 3)
  
  # Hexbins counts
  # number of bins
  n_bins.pb[i] = nrow(pb_pdata.2)
  n_bins.gb[i] = nrow(gb_pdata.2)
  
  # number of studied bins
  s_bins.pb[i] = sum(pb_pdata.3$value > 0)
  s_bins.gb[i] = sum(gb_pdata.3$value > 0)
}

var_pb = data.frame(n_bins = n_bins.pb, studied_bins = s_bins.pb, 
                    dataset = "Phoible", label = bin_size)
var_gb = data.frame(n_bins = n_bins.gb, studied_bins = s_bins.gb, 
                    dataset = "Grambank", label = bin_size)
var_df = rbind(var_pb, var_gb)

ggplot(var_df, aes(x = n_bins, y = studied_bins, col = dataset, label = label)) + 
  geom_text() + 
  ylab("Number of studied bins") + 
