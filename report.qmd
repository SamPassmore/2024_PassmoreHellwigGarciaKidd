...
EGIDS is a 13-point scale of endangerment for 7,206 languages, ranging from 1 (Internationally used languages) to 10 (Langauge is no longer used, and no one holds a symbolic sense of identity with the language). There are seven categories of 'safe' languages, a vulnerable category, three increasing levels of endangerment, and two categories of languages that are no longer used. Our data collapses the two sleeping categories.
...
All levels are detailed in the table below:
...
languages$nochildren = ifelse(languages$EGIDS %in% nochildren_levels, 1, 0)
languages$nochildren_strict = ifelse(languages$EGIDS %in% nochildren_levels2, 1, 0)
```
...
```{r, glottolog, message=FALSE}
glottolog =
  cldf("https://github.com/glottolog/glottolog-cldf/archive/refs/tags/v4.8.zip")

glottolog_languages = glottolog$tables$LanguageTable
```
...
## The data is in long format so if a paper is about morphosyntax and pragmatics in English, 
## English will be listed across two rows. 
kiddgarcia_counts = kiddgarcia %>% 
  distinct(title, year, volume, issue, journal, egids_matching, .keep_all = TRUE) %>% 
...
p_base = ggplot(plot_prop,
  ...
  xlab("EGIDS") +
  scale_fill_manual(values = c(rep("#00b38a", 7), "#f2ac42", rep("#ea324c", 4))) +
...
The majority of unstudied languages in the Kidd & Garcia (2022) data set are categorised as written, vigorous, or threatened (`r round(unstud_prop, 2) * 100`%).
...
p1_phoible = ggplot(plot_phoible, 
  ...
  geom_point(aes(shape = factor(studied), size = phonology_sum + 1), alpha = 0.5) +
  coord_equal() + 
  theme_classic(base_size = font_size) + 
...
p1_grambank = ggplot(plot_grambank, 
                     aes(MDS.X, MDS.Y, col = factor(children))) + 
  geom_point(aes(shape = factor(studied), size = morphosyntax_sum + 1), alpha = 0.5) +
  coord_equal() + 
  theme_classic(base_size = font_size) + 
  theme(aspect.ratio = 1, legend.position = "right") + 
  scale_linewidth(range = c(0, 3)) + 
...
ggplot(data = languages, aes(y = L1_POP, x = factor(studied), fill = factor(studied))) + 
  geom_boxplot() +
  geom_jitter() + 
  scale_y_log10() + 
  theme_classic()
